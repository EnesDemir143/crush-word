...
void main() {
  ...
  group('empty state', () {
    testWidgets(
      'shows empty message when no results exist',
      (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: ScoreHistoryScreen(controller: controller),
          ),
        );
        await tester.pumpAndSettle();

        expect(
          find.byKey(const Key('history-empty')),
          findsOneWidget,
        );
      },
    );
  ...
  group('with saved results', () {
    ...
    testWidgets(
      'renders newest-first game cards with required fields',
      (WidgetTester tester) async {
        ...
        expect(
          find.byKey(const Key('history-summary')),
          findsOneWidget,
        );

        // All three game cards are present
        expect(
          find.byKey(const Key('history-card-game-3')),
          findsOneWidget,
        );
        expect(
          find.byKey(const Key('history-card-game-2')),
          findsOneWidget,
        );
        expect(
          find.byKey(const Key('history-card-game-1')),
          findsOneWidget,
        );
        ...
        expect(
          ...
          lessThan(card1Y),
          reason:
              'The newest game must appear above the oldest',
        );
    ...
    testWidgets(
      'summary section shows correct aggregate metrics',
      (WidgetTester tester) async {
        repository.results = testResults;
        ...
        expect(
          find.descendant(
            of: find.byKey(
              const Key('summary-total-duration'),
            ),
